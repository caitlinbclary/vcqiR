...
check_analysis_metadata <- function(VCP = "check_analysis_metadata"){
  ...
  for(i in 1:length(programlist)){
    if(vcqi_object_exists(programlist[i]) == FALSE){
      errormsgs <- c(errormsgs, paste0("Please define ", programlist[i], " in the control program."))
      vcqi_log_comment(VCP, 1, "Error", paste0("Please define ", programlist[i], " in the control program."))
      exitflag <- 1
    }
  }
  ...
  if(vcqi_object_exists("VCQI_LEVEL4_SET_VARLIST")){
    ...
    if(exitflag != 1  & !vcqi_object_exists("VCQI_LEVEL4_SET_LAYOUT")){
      ...
      #2023-11-15 update: add format columns with default settings
